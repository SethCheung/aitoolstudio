...
function injectMinimapCSS() {
  ...
  style.textContent = `
    ...
    .minimap-empty { position:absolute; inset:0; display:flex; align-items:center; justify-content:center; color:var(--faint); font-size:10px; font-weight:600; letter-spacing:.14em; pointer-events:none; opacity:.7; }
    .minimap.is-empty { opacity:.45; transition:opacity .25s ease; }
    .minimap.is-empty:hover { opacity:.9; }
...
function renderMinimap() {
  ...
  minimapContent.innerHTML = `${nodeHtml}${nodes?.length ? '' : '<div class="minimap-empty">空画布</div>'}<div id="minimapViewport" class="minimap-viewport"></div>`;
  const minimapBox = minimapContent.closest('.minimap');
  if (minimapBox) minimapBox.classList.toggle('is-empty', !(nodes && nodes.length));
