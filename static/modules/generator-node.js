...
function renderGeneratorBody(node) {
    ...
    wrap.innerHTML = `
        ...
        ${generatorNodeDeps.runErrorHtml ? generatorNodeDeps.runErrorHtml(node) : ''}
...
async function runGenerator(genId, opts = {}) {
    ...
    if (!refs.length && String(generatorNodeDeps.resolveImageModel(gen.model) || '').toLowerCase().includes('edit')) {
        if (!cascade) {
            gen.runStatus = 'failed';
