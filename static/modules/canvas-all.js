...
function refreshIcons(){ if(window.lucide) lucide.createIcons(); }
        ...
        function runErrorHtml(node){
            if(node.runStatus !== 'failed' || node._cascadeFailed || !node.runError) return '';
            return `<div class="node-retry-bar node-error-bar" data-error-bar>
                <span class="node-retry-msg" title="${escapeAttr(node.runError||'')}">${escapeHtml((node.runError||'生成失败').slice(0,80))}</span>
            </div>`;
        }
        ...
        async function loadConfig(){
            ...
            try {
                ...
                try {
                    // 只列「已发布」工作流：与 ComfyUI 工作台发布动作对齐，避免草稿/垃圾文件混进画布
                    const wf = await fetch('/api/workflows-public').then(r=>r.json());
        ...
        async function runMsGenNode(nodeId, opts={}){
            ...
            if(!prompt){
                if(!cascade) {
                    ...
                    node.runError = '请先连接一个有内容的提示词节点，说明要如何生成或编辑。';
                    render();
                    scheduleSave();
                    alert(node.runError);
                }
            ...
            if(msModel.supportsImage && !refs.length){
                if(!cascade) {
                    node.runStatus = 'failed';
                    node.runError = '当前模型需要输入图片，请同时连接图片和提示词。';
                    render();
                    scheduleSave();
                    alert(node.runError);
                }
        ...
        async function runGenerator(genId, opts={}){
            ...
            if(!prompt){
                if(!cascade) {
                    ...
                    gen.runError = '请先连接一个有内容的提示词节点，说明要如何生成或编辑。';
                    render();
                    scheduleSave();
                    alert(gen.runError);
                }
            ...
            if(!refs.length && String(resolveImageModel(gen.model) || '').toLowerCase().includes('edit')){
                if(!cascade) {
                    gen.runStatus = 'failed';
                    gen.runError = '当前模型是图片编辑模型，请同时连接图片和提示词。';
                    render();
                    scheduleSave();
                    alert(gen.runError);
                }
        ...
        function triggerGeneratorButtonRun(e){
            const btn = e.target?.closest?.('.gen-btn');
            if(!btn || btn.disabled) return;
            const nodeEl = btn.closest('.node');
            if(!nodeEl) return;
            const node = nodes.find(n => n.id === nodeEl.dataset.id);
            if(!node || !['generator', 'msgen'].includes(node.type)) return;
            const now = Date.now();
            const last = Number(btn.dataset.lastRunTrigger || 0);
            if(now - last < 800) return;
            btn.dataset.lastRunTrigger = String(now);
            e.preventDefault();
            e.stopPropagation();
            if(typeof e.stopImmediatePropagation === 'function') e.stopImmediatePropagation();
            if(node.type === 'generator') runGenerator(node.id);
            else runMsGenNode(node.id);
        }
        ...
        async function runCustomWorkflow(nodeId) {
            ...
            // pendingId 必须声明在 try 外：catch 里要用它清理进度状态（之前 const 在 try 内，
            // 失败路径直接 ReferenceError，导致节点永远卡在“处理中/0%”且不弹错误）
            let pendingId = null;
            try {
                // 加载工作流配置
                const wfData = comfyWorkflowCache[node.workflowId];
                ...
                try {
                    ...
                    const wsUrl = `${wsProtocol}//${window.location.host}/api/comfyui/ws?client_id=${encodeURIComponent(CLIENT_ID)}`;
                    try {
        ...
        document.addEventListener('pointerup', triggerGeneratorButtonRun, true);
        document.addEventListener('click', triggerGeneratorButtonRun, true);
        board.onwheel = e => {
            if(!window.canvas) return;
            e.preventDefault();
        ...
        window.onload = async () => {
            ...
            await loadConfig();
            await loadCanvasList(false);
            // 深链支持：/canvas?id=xxx 直接打开指定画布（来自项目主页“打开”按钮等入口）
            const deepLinkCanvasId = new URLSearchParams(location.search).get('id');
            if (deepLinkCanvasId) {
                await openCanvas(deepLinkCanvasId);
            } else {
                setCanvasMode(false);
            }
