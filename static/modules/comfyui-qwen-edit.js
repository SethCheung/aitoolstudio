...
async function qwenEditExecute(node, refs, prompt, promptId) {
    ...
    const promptText = node.qePrompt || prompt || "将摄像机向右旋转45度";
    const seed = Number(node.qeSeed || Math.floor(Math.random() * 10**15));

    const localResult = await fetch('/api/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            prompt: promptText,
            workflow_json: 'custom/local-view-2511.json',
            type: 'qwen_edit',
            params: {
                "31": { image: inputName },
                "11": { prompt: promptText },
                "14": { seed }
            },
            client_id: CLIENT_ID,
    ...
    if (localResult.error) throw new Error(`视角转换失败：${localResult.error}`);
    if (!localResult.images?.length) throw new Error('视角转换失败：未返回图片');
    return { images: localResult.images || [] };
