...
class AuthManager {
    ...
    static async verify() {
        ...
        try {
            ...
            this.setUser({ username: u.username, is_admin: u.is_admin === true });
            this.setToken(token);
            return true;
        } catch (e) {
            return false;
...
class SettingsUI {
    ...
    async showUserList() {
        ...
        let html = `
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
                ...
                <div style="display:flex;align-items:center;gap:8px;">
                    <button id="users-panel-add" style="height:32px;padding:0 14px;border-radius:8px;border:none;background:linear-gradient(135deg,#00b4d8,#00f0ff);color:#0a1628;font-size:12px;font-weight:700;cursor:pointer;transition:all 0.2s;">+ 新增用户</button>
            ...
            html += `
                <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 0;border-bottom:1px solid var(--border);">
                    <div style="display:flex;align-items:center;gap:10px;">
                        ...
                        ${u.is_disabled ? '<span style="font-size:11px;color:#ef4444;background:rgba(239,68,68,0.12);padding:2px 8px;border-radius:8px;">已禁用</span>' : ''}
                    </div>
                    <div style="display:flex;gap:6px;">
                        <button class="user-reset-btn" data-username="${this.escapeHtml(u.username)}" data-id="${u.id}" style="height:32px;padding:0 14px;border-radius:8px;border:1px solid var(--border);background:transparent;color:var(--text);font-size:12px;font-weight:600;cursor:pointer;transition:all 0.2s;">重置密码</button>
                        ${canDelete ? `<button class="user-delete-btn" data-username="${this.escapeHtml(u.username)}" data-id="${u.id}" style="height:32px;padding:0 14px;border-radius:8px;border:1px solid #ef4444;background:transparent;color:#ef4444;font-size:12px;font-weight:600;cursor:pointer;transition:all 0.2s;">禁用</button>` : ''}
                    </div>
                </div>
            `;
        });
        ...
        document.getElementById('users-panel-add').onclick = () => this.showCreateUserModal(overlay);
        overlay.addEventListener('click', (e) => { if (e.target === overlay) overlay.remove(); });

        overlay.querySelectorAll('.user-reset-btn').forEach(btn => {
            btn.onclick = () => {
                const username = btn.dataset.username;
                this.showResetPasswordModal(username, btn.dataset.id, overlay);
            };
        });
        ...
        overlay.querySelectorAll('.user-delete-btn').forEach(btn => {
            btn.onclick = () => {
                const username = btn.dataset.username;
                this.showDeleteConfirmModal(username, btn.dataset.id, overlay);
            };
        });
    }

    showCreateUserModal(parentOverlay) {
        const modal = document.createElement('div');
        modal.style.cssText = `
            position: fixed; inset: 0; z-index: 2100;
            display: flex; align-items: center; justify-content: center;
        ...
        modal.innerHTML = `
            <div style="width:320px;background:var(--sidebar-bg);border:1px solid var(--border);border-radius:20px;padding:24px;box-shadow:0 20px 60px rgba(0,0,0,0.2);">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
                    <h2 style="margin:0;font-size:16px;font-weight:700;color:var(--text);">新增用户</h2>
                    <button id="create-modal-close" style="background:none;border:none;color:var(--muted);cursor:pointer;padding:4px;">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                    </button>
                </div>
                <input id="create-username" type="text" placeholder="用户名" autocomplete="off" style="width:100%;height:42px;padding:0 14px;border-radius:10px;border:1px solid var(--border);background:var(--stage-bg);color:var(--text);font-size:14px;outline:none;box-sizing:border-box;margin-bottom:12px;">
                <input id="create-password" type="password" placeholder="初始密码（至少 6 位）" autocomplete="new-password" style="width:100%;height:42px;padding:0 14px;border-radius:10px;border:1px solid var(--border);background:var(--stage-bg);color:var(--text);font-size:14px;outline:none;box-sizing:border-box;">
                <label style="display:flex;align-items:center;gap:8px;margin-top:14px;font-size:13px;color:var(--text);cursor:pointer;">
                    <input id="create-is-admin" type="checkbox" style="width:16px;height:16px;accent-color:#00b4d8;cursor:pointer;">
                    设为管理员
                </label>
                <div id="create-error" style="color:#ef4444;font-size:12px;margin:12px 0 0;display:none;"></div>
                <button id="create-confirm-btn" style="width:100%;height:42px;border-radius:10px;border:none;background:linear-gradient(135deg,#00b4d8,#00f0ff);color:#0a1628;font-size:13px;font-weight:700;cursor:pointer;margin-top:16px;transition:all 0.2s;">创建用户</button>
            </div>
        `;
        document.body.appendChild(modal);
        const usernameInput = modal.querySelector('#create-username');
        const passwordInput = modal.querySelector('#create-password');
        const errorEl = modal.querySelector('#create-error');
        const confirmBtn = modal.querySelector('#create-confirm-btn');
        const showError = (msg) => { errorEl.textContent = msg; errorEl.style.display = 'block'; };
        modal.querySelector('#create-modal-close').onclick = () => modal.remove();
        ...
        confirmBtn.onclick = async () => {
            ...
            try {
                const resp = await fetch('/api/auth/users', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ username, password, is_admin: modal.querySelector('#create-is-admin').checked })
                });
                if (!resp.ok) {
                ...
                confirmBtn.textContent = '创建用户';
            }
        };
        passwordInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') confirmBtn.click(); });
    }

    showResetPasswordModal(username, userId, parentOverlay) {
        const modal = document.createElement('div');
        modal.style.cssText = `
            position: fixed; inset: 0; z-index: 2100;
            display: flex; align-items: center; justify-content: center;
        ...
        document.getElementById('reset-confirm-btn').onclick = async () => {
            ...
            try {
                const resp = await fetch('/api/auth/users/' + encodeURIComponent(userId) + '/reset-password', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    showDeleteConfirmModal(username, userId, parentOverlay) {
        const modal = document.createElement('div');
        modal.style.cssText = `
            position: fixed; inset: 0; z-index: 2100;
            display: flex; align-items: center; justify-content: center;
        ...
        modal.innerHTML = `
            <div style="width:320px;background:var(--sidebar-bg);border:1px solid var(--border);border-radius:20px;padding:24px;box-shadow:0 20px 60px rgba(0,0,0,0.2);">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
                    <h2 style="margin:0;font-size:16px;font-weight:700;color:var(--text);">禁用账号</h2>
                    <button id="delete-modal-close" style="background:none;border:none;color:var(--muted);cursor:pointer;padding:4px;">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                    </button>
                </div>
                <p style="font-size:13px;color:var(--muted);margin-bottom:20px;">确定要禁用用户 <strong style="color:var(--text);">${this.escapeHtml(username)}</strong> 吗？禁用后无法登录，可由管理员重新启用。</p>
                <div style="display:flex;gap:10px;">
                    ...
                    <button id="delete-confirm-btn" style="flex:1;height:40px;border-radius:10px;border:none;background:#ef4444;color:#fff;font-size:13px;font-weight:700;cursor:pointer;">确认禁用</button>
                </div>
        ...
        document.getElementById('delete-confirm-btn').onclick = async () => {
            try {
                ...
                if (!resp.ok) {
                    const err = await resp.json();
                    throw new Error(err.detail || '禁用失败');
                }
                modal.remove();
                parentOverlay.remove();
                this.showToast('账号已禁用');
            } catch (e) {
