...
const rhFreeKeyStatusBadge = document.getElementById('rhFreeKeyStatusBadge');
const rhFreeKeyMaskedText = document.getElementById('rhFreeKeyMaskedText');
const rhWalletKeyStatusBadge = document.getElementById('rhWalletKeyStatusBadge');
const rhWalletKeyMaskedText = document.getElementById('rhWalletKeyMaskedText');
...
function rhFreeKeyHintText(item){
    return item?.has_key
        ? `当前 RH币 Key 已保存到 ${item.key_env || 'API/.env'}（输入框不会回显完整 Key）`
        : '还没有保存 RH币 Key。';
}
function rhWalletKeyHintText(item){
    return item?.has_wallet_key
        ? `当前账户余额 Key 已保存到 ${item.wallet_key_env || 'API/.env'}（输入框不会回显完整 Key）`
        : '还没有保存账户余额 Key。验证地址和拉取模型会优先使用它。';
}
function updateKeyStatus(el, maskedEl, configured, masked){
    if(!el || !maskedEl) return;
    const on = !!configured;
    el.textContent = on ? '已配置' : '未配置';
    el.classList.toggle('is-on', on);
    el.classList.toggle('is-off', !on);
    maskedEl.textContent = `masked_key: ${String(masked || '-').trim() || '-'}`;
}
function providerMaskedKey(item){
    return item?.masked_key || item?.key_preview || '';
}
function providerMaskedWalletKey(item){
    return item?.masked_wallet_key || item?.wallet_key_preview || '';
}
...
function renderEditor(){
    ...
    keyInput.value = '';
    const maskedKey = providerMaskedKey(item);
    const maskedWalletKey = providerMaskedWalletKey(item);
    keyInput.placeholder = item.has_key ? `${tr('api.keepCurrentKey')} ${maskedKey}` : tr('api.enterKey');
    keyHint.textContent = item.has_key ? `${tr('api.keySaved')}${item.key_env || 'API/.env'}（输入框不会回显完整 Key）` : tr('api.noKey');
    updateKeyStatus(keyStatusBadge, keyMaskedText, item.has_key, maskedKey);
    ...
    if(isRunningHub){
        ...
        if(rhFreeKeyInput){
            ...
            rhFreeKeyInput.placeholder = item.has_key ? `保持当前 RH币 Key ${maskedKey}` : '输入 RH币 API Key';
        }
        if(rhWalletKeyInput){
            rhWalletKeyInput.value = '';
            rhWalletKeyInput.placeholder = item.has_wallet_key ? `保持当前账户余额 Key ${maskedWalletKey}` : '输入账户余额 API Key';
        }
        ...
        updateKeyStatus(rhFreeKeyStatusBadge, rhFreeKeyMaskedText, item.has_key, maskedKey);
        updateKeyStatus(rhWalletKeyStatusBadge, rhWalletKeyMaskedText, item.has_wallet_key, maskedWalletKey);
        renderRunningHubCards();
    }
...
function addProvider(){
    ...
    providers.push({id, name:'API', base_url:'https://', protocol:'openai', image_generation_endpoint:'', image_edit_endpoint:'', enabled:true, primary:false, image_models:[], chat_models:[], video_models:[], has_key:false, masked_key:''});
    selectedId = id;
    renderEditor();
}
