...
const LTX_DIRECTOR_FIELD_IDS = {
    global_prompt:'f_global_prompt',
    duration_frames:'f_duration_frames',
    duration_seconds:'f_duration_seconds',
    timeline_data:'f_timeline_data',
    use_custom_audio:'f_use_custom_audio',
    frame_rate:'f_frame_rate',
    custom_width:'f_custom_width',
    custom_height:'f_custom_height',
    resize_method:'f_4t0z0g8',
    noise_seed:'f_noise_seed',
};
...
const REQUESTED_CANVAS_ID = (PAGE_PARAMS.get('id') || '').trim();
const REQUESTED_CANVAS_BOOTSTRAP_CLASS = 'canvas-requested-open';
let requestedCanvasBootstrapPending = Boolean(REQUESTED_CANVAS_ID);
function setRequestedCanvasBootstrapPending(active){
    requestedCanvasBootstrapPending = Boolean(active && REQUESTED_CANVAS_ID);
    const openRequested = requestedCanvasBootstrapPending;
    document.documentElement.classList.toggle(REQUESTED_CANVAS_BOOTSTRAP_CLASS, openRequested);
    document.body?.classList.toggle(REQUESTED_CANVAS_BOOTSTRAP_CLASS, openRequested);
}
setRequestedCanvasBootstrapPending(requestedCanvasBootstrapPending);
function resolveRequestedCanvasId(){
    const requested = String(REQUESTED_CANVAS_ID || '').trim();
    if(!requested) return '';
    const direct = canvases.find(item => item?.id === requested);
    if(direct?.id) return direct.id;
    const mapped = canvases.find(item => item?.project_id === requested);
    return mapped?.id || requested;
}
function requestedCanvasOpenError(status, detail=''){
    const cleanDetail = String(detail || '').trim();
    if(status === 401){
        return langIsEn() ? 'Project cannot be opened: session expired, please sign in again.' : '项目无法打开：登录状态已失效，请重新登录。';
    }
    if(status === 403){
        return langIsEn() ? 'Project cannot be opened: you do not have access.' : '项目无法打开：你没有访问权限。';
    }
    if(status === 404){
        return langIsEn() ? 'Project cannot be opened: it was deleted or no longer exists.' : '项目无法打开：项目不存在或已删除。';
    }
    if(cleanDetail){
        return langIsEn() ? `Project cannot be opened: ${cleanDetail}` : `项目无法打开：${cleanDetail}`;
    }
    return langIsEn() ? 'Project cannot be opened, please return to project list.' : '项目无法打开，请返回项目列表后重试。';
}
function showRequestedCanvasOpenError(status, detail=''){
    if(gateTitleText) gateTitleText.textContent = langIsEn() ? 'Cannot Open Project' : '项目无法打开';
    if(gateSubtitle) gateSubtitle.textContent = requestedCanvasOpenError(status, detail);
    setStatus(requestedCanvasOpenError(status, detail));
}
...
function projectToCanvasItem(project){
    const meta = project?.default_canvas || {};
    const id = meta.id || project?.default_canvas_id || project?.id || '';
    const kind = normalizeCanvasKind(meta.kind || project?.kind || 'classic');
    return {
        ...meta,
        id,
        project_id: project?.id || meta.project_id || '',
        title: project?.title || meta.title || tr('canvas.untitledProject'),
        icon: meta.icon || project?.icon || (kind === 'smart' ? 'sparkles' : '🧩'),
        kind,
        owner_user_id: Number(project?.owner_user_id || meta.owner_user_id || 0),
        owner: project?.owner || null,
        thumbnail_url: project?.thumbnail_url || '',
        created_at: Number(project?.created_at || meta.created_at || 0),
        updated_at: Math.max(Number(project?.updated_at || 0), Number(meta.updated_at || 0)),
        deleted_at: Number(project?.archived_at || meta.deleted_at || 0),
        node_count: Number(project?.node_count || meta.node_count || 0),
    };
}
function normalizeCanvasKind(kind){
    return String(kind || '').toLowerCase() === 'smart' ? 'smart' : 'classic';
}
function projectApiBaseForCanvas(id){
    const item = [...canvases, ...deletedCanvases].find(entry => entry.id === id);
    return item?.project_id ? `/api/projects/${encodeURIComponent(item.project_id)}` : `/api/canvases/${encodeURIComponent(id)}`;
}
...
function setCanvasMode(open){
    ...
    if(canvasGate) canvasGate.style.display = open ? 'none' : 'flex';
    if(!open){
        nodesEl.innerHTML = '';
        linksEl.innerHTML = '';
...
async function loadConfig(){
    ...
    try {
        ...
        comfyWorkflows = await fetchComfyWorkflowList();
...
async function fetchComfyWorkflowList(){
    for(const url of ['/api/workflows-public', '/api/workflows']){
        try {
...
async function fetchComfyWorkflowDetail(name){
    const encoded = encodeURIComponent(name);
    for(const url of [`/api/workflows-public/${encoded}`, `/api/workflows/${encoded}`]){
        try {
            const res = await fetch(url);
            if(!res.ok) continue;
...
async function loadCanvasList(openFirst=true){
    try {
        ...
        if(!res.ok) throw new Error(tr('canvas.projectListFailed'));
        const data = await res.json();
        canvases = (data.projects || []).map(projectToCanvasItem).filter(item => item.id);
        refreshGateViewControls();
        renderCanvasList();
        refreshTrashCount();
        if(openFirst && canvases[0]) await openCanvas(canvases[0].id);
        else if(!canvas && !requestedCanvasBootstrapPending) {
            setCanvasMode(false);
        ...
        setStatus(tr('canvas.projectListFailed'));
        console.error(e);
    }
}
async function loadTrashList(){
    try {
        ...
        const data = await res.json();
        deletedCanvases = (data.projects || []).map(projectToCanvasItem).filter(item => item.id);
        refreshGateViewControls();
        renderCanvasList();
...
async function createCanvas(){
    ...
    try {
        const res = await fetch('/api/projects', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({title, kind:isSmart ? 'smart' : 'classic'})
        });
        if(!res.ok) throw new Error(tr('canvas.createFailed'));
        const data = await res.json();
        const createdCanvas = data.canvas || data.project?.default_canvas || null;
        if(!createdCanvas?.id) throw new Error(tr('canvas.createFailed'));
        if(isSmart){
            setCreateMode(false);
            await loadCanvasList(false);
            openSmartCanvasPage(createdCanvas?.id || data.project?.default_canvas_id);
            return;
        }
...
function openSmartCanvasPage(id){
    if(!id) return;
    window.location.href = `/smart-canvas?id=${encodeURIComponent(id)}`;
}
...
async function setCanvasIcon(id, icon, event){
    ...
    try {
        if(item?.project_id){
            const res = await fetch(`/api/projects/${encodeURIComponent(item.project_id)}`, {
                method:'PATCH',
                headers:{'Content-Type':'application/json'},
                body:JSON.stringify({icon:icon || 'layers'})
            });
            if(!res.ok) throw new Error('图标保存失败');
            if(canvas?.id === id) canvas.icon = icon || 'layers';
            await loadCanvasList(false);
            return;
        }
...
async function openCanvas(id){
    ...
    try {
        const res = await fetch(`/api/canvases/${id}`);
        if(!res.ok){
            const err = new Error(await responseErrorMessage(res, tr('canvas.openFailed')));
        ...
        if(canvas.id && window.location.pathname === '/canvas'){
            const nextUrl = `/canvas?id=${encodeURIComponent(canvas.id)}`;
        ...
        setStatus(e?.message || tr('canvas.openFailed'));
        console.error(e);
        return {ok:false, status:Number(e?.status || 0), message:String(e?.message || tr('canvas.openFailed'))};
    }
}
...
async function deleteCanvas(id, event){
    ...
    try {
        const res = await fetch(projectApiBaseForCanvas(id), {method:'DELETE'});
...
async function restoreCanvas(id, event){
    ...
    setStatus('Restoring...');
    try {
        const res = await fetch(`${projectApiBaseForCanvas(id)}/restore`, {method:'POST'});
...
async function purgeCanvas(id, event){
    ...
    setStatus('Deleting...');
    try {
        const res = await fetch(`${projectApiBaseForCanvas(id)}/purge`, {method:'DELETE'});
...
backToManagerBtn.addEventListener('click', backToProjects);
...
async function ensureComfyWorkflow(name){
    ...
    const data = await fetchComfyWorkflowDetail(name);
    if(!data){
...
function comfyPrimaryPromptId(fields=[]){
    return (fields || []).find(f => comfyFieldKind(f) === 'prompt')?.id || '';
}
function comfySettingFields(fields=[]){
    const primaryPromptId = comfyPrimaryPromptId(fields);
    return (fields || []).filter(f => {
        const kind = comfyFieldKind(f);
        return kind === 'setting' || (kind === 'prompt' && f.id !== primaryPromptId);
    });
}
...
function renderComfySettings(container, node){
    ...
    if(mode === 'text'){
        ...
        const data = currentComfyWorkflow(node);
        const fields = data?.config?.fields || [];
        const settingFields = comfySettingFields(fields);
        container.innerHTML = `
            <div class="gen-settings-row">
...
async function runLTXDirectorNode(nodeId, opts={}){
    ...
    try {
        ...
        const workflow = await ensureComfyWorkflow(LTX_DIRECTOR_WORKFLOW);
        if(!workflow) throw new Error(tr('canvas.comfyNoWorkflow'));
        const fields = workflow.config?.fields || [];
        const values = {};
        const setWorkflowValue = (fieldId, value) => {
            const field = fields.find(f => f.id === fieldId);
            if(field) values[field.id] = value;
        };
        Object.entries(directorInputs).forEach(([key, value]) => setWorkflowValue(LTX_DIRECTOR_FIELD_IDS[key], value));
        setWorkflowValue(LTX_DIRECTOR_FIELD_IDS.noise_seed, Number(node.noiseSeed ?? 12));
        const result = await cascadeFetch(`/api/workflows/${encodeURIComponent(LTX_DIRECTOR_WORKFLOW)}/run`, {
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({
                fields:values,
...
async function runComfyNode(nodeId, opts={}){
    ...
    const requiredImageFields = customImageFields.filter(f => f.required === true);
    const requiredVideoFields = customVideoFields.filter(f => f.required === true);
    const requiredAudioFields = customAudioFields.filter(f => f.required === true);
    if((mode === 'text' || (mode === 'custom' && customPromptFields.length)) && !prompt){ alert(tr('canvas.needPrompt')); return; }
    if((mode !== 'text' && mode !== 'custom' && !refs.length) || (mode === 'custom' && refs.length < requiredImageFields.length)){ alert(tr('canvas.needImage')); return; }
    if(mode === 'custom' && videoRefsOnly(allRefs).length < requiredVideoFields.length){ alert(langIsEn() ? 'Please connect enough video inputs for this ComfyUI workflow.' : '请为这个 ComfyUI 工作流连接足够的视频输入'); return; }
    if(mode === 'custom' && audioRefsOnly(allRefs).length < requiredAudioFields.length){ alert(langIsEn() ? 'Please connect enough audio inputs for this ComfyUI workflow.' : '请为这个 ComfyUI 工作流连接足够的音频输入'); return; }
    ...
    try {
        ...
        if(mode === 'text'){
            ...
            promptFields.forEach((f, index) => {
                values[f.id] = index === 0 ? prompt : comfyParamValue(node, f);
            });
            settingFields.forEach(f => {
                ...
                values[f.id] = comfyParamValue(node, f);
            });
            const result = await cascadeFetch(`/api/workflows/${encodeURIComponent(workflowName)}/run`, {
                method:'POST',
                headers:{'Content-Type':'application/json'},
                body:JSON.stringify({
                    fields:values,
...
window.onload = async () => {
    ...
    if(REQUESTED_CANVAS_ID){
        await loadCanvasList(false);
        const openResult = await openCanvas(resolveRequestedCanvasId());
        setRequestedCanvasBootstrapPending(false);
        if(!openResult?.ok){
            setCanvasMode(false);
            showRequestedCanvasOpenError(openResult?.status || 0, openResult?.message || '');
        }
        return;
    }
    setRequestedCanvasBootstrapPending(false);
