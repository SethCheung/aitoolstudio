...
let canvasId = (params.get('id') || '').trim();
const CLIENT_ID = 'smart_canvas_' + Math.random().toString(36).slice(2);
...
function backToCanvasList(){ savePromptDraftForCurrent(); window.location.href = '/projects'; }
...
function smartComfyPrimaryPromptId(fields=[]){
    return (fields || []).find(f => comfyFieldKind(f) === 'prompt')?.id || '';
}
function smartComfySettingFields(fields=[]){
    const primaryPromptId = smartComfyPrimaryPromptId(fields);
...
function smartComfyValueMissing(value){
    return value === undefined || value === null || value === '';
}
function assertRequiredComfyFields(fields, values){
    const missing = (fields || []).filter(field => field.required === true && smartComfyValueMissing(values[field.id]));
    if(missing.length){
        const names = missing.map(field => field.name || field.input || field.id).join('、');
        throw new Error(`ComfyUI 工作流缺少必填参数：${names}`);
    }
}
...
function renderComfyParams(){
    ...
    if(settings.comfyMode === 'text'){
        ...
        const fields = smartComfySettingFields(wf?.config?.fields || []);
...
async function resolveSmartCanvasIdFromProject(projectId){
    ...
    try {
        ...
        if(!res.ok) return '';
        const data = await res.json();
        const project = (data.projects || []).find(item => String(item?.id || '').trim() === requestedProjectId);
        if(!project) return '';
        const defaultCanvas = project.default_canvas || {};
        return String(defaultCanvas.id || project.default_canvas_id || '').trim();
    } catch(_e) {
        return '';
    }
}
async function loadCanvas(){
    ...
    try {
        ...
        let res = await fetch(`/api/canvases/${encodeURIComponent(targetCanvasId)}`);
        if(!res.ok && res.status === 404){
            const fallbackCanvasId = await resolveSmartCanvasIdFromProject(targetCanvasId);
            if(fallbackCanvasId && fallbackCanvasId !== targetCanvasId){
                targetCanvasId = fallbackCanvasId;
                res = await fetch(`/api/canvases/${encodeURIComponent(targetCanvasId)}`);
            }
        }
        if(!res.ok) return;
        canvasId = targetCanvasId;
        if(canvasId && window.location.pathname === '/smart-canvas'){
            const nextUrl = `/smart-canvas?id=${encodeURIComponent(canvasId)}`;
    ...
    if(settings.engine === 'comfy'){
        ...
        if(mode === 'text'){
            const data = await fetch('/api/generate', {
                method:'POST',
                headers:{'Content-Type':'application/json'},
                body:JSON.stringify({prompt, width:Number(settings.width || 1024), height:Number(settings.height || 1024), workflow_json:'Z-Image.json', type:'zimage', client_id:CLIENT_ID})
        ...
        if(mode === 'enhance'){
            ...
            const data = await fetch('/api/generate', {
                method:'POST',
                headers:{'Content-Type':'application/json'},
                body:JSON.stringify({workflow_json:'Z-Image-Enhance.json', type:'enhance', params:{"15":{image:inputName},"204":{value:Number(settings.enhanceStrength ?? 0.5)}}, client_id:CLIENT_ID})
            }).then(async r => { if(!r.ok) throw new Error(await r.text()); return r.json(); });
            const urls = resultMediaUrls(data);
            return {urls, kind:mediaKindForUrls(urls, 'image')};
        }
        if(mode === 'edit'){
            ...
            const data = await fetch('/api/generate', {
                method:'POST',
                headers:{'Content-Type':'application/json'},
                body:JSON.stringify({prompt, workflow_json:'Flux2-Klein.json', type:'klein', params:{"168":{text:prompt},"158":{noise_seed:Math.floor(Math.random()*1000000)},"278":{image:names[0] || ""},"270":{image:names[1] || ""},"292":{image:names[2] || ""},"313":{value:Boolean(names[1])},"314":{value:Boolean(names[2])}}, client_id:CLIENT_ID})
            }).then(async r => { if(!r.ok) throw new Error(await r.text()); return r.json(); });
            const urls = resultMediaUrls(data);
            return {urls, kind:mediaKindForUrls(urls, 'image')};
        }
        ...
        const wf = await fetchComfyWorkflowDetail(workflowName);
        if(!wf) throw new Error(tr('smart.errNeedWorkflow'));
        const fields = wf.config?.fields || [];
        const values = {};
        fields.filter(f => comfyFieldKind(f) === 'prompt').forEach((field, index) => {
            values[field.id] = index === 0 ? prompt : (settings.comfyParams?.[field.id] ?? field.default ?? '');
        });
        ...
        assertRequiredComfyFields(fields, values);
        const result = await fetch(`/api/workflows/${encodeURIComponent(workflowName)}/run`, {
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({fields:values, client_id:CLIENT_ID})
        }).then(async r => { if(!r.ok) throw new Error(await r.text()); return r.json(); });
