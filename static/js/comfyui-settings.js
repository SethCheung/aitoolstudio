...
let currentConfig = null;       // { title, enabled, category, description, thumbnail, fields:[...] }
let isBuiltin = false;
...
let workflowInstallPollTimer = null;
let workflowAutoDownloadSummary = null;
...
const importWorkflowNameInput = document.getElementById('importWorkflowNameInput');
const importWizardResultEl = document.getElementById('importWizardResult');
const resourceRootInput = document.getElementById('resourceRootInput');
const resourceRootSummaryEl = document.getElementById('resourceRootSummary');
const resourceRootDirsEl = document.getElementById('resourceRootDirs');
let resourceRootState = null;
...
function formatDownloadSpeed(bytesPerSec){
    ...
    return `${formatDownloadBytes(n)}/s`;
}
...
function resourceDirStatusClass(status){
    if(status === 'ok') return 'ok';
    if(status === 'error' || status === 'invalid' || status === 'unreadable') return 'error';
    return 'missing';
}
function resourceDirStatusLabel(status){
...
function normalizeConfig(cfg, fallbackTitle=''){
    cfg = cfg && typeof cfg === 'object' ? cfg : {};
    const fields = Array.isArray(cfg.fields) ? cfg.fields.map(f => ({
        enabled: true,
        hidden: false,
        required: false,
        options: [],
    ...
    return {
        ...
        fields,
        mini_cards: cfg.mini_cards && typeof cfg.mini_cards === 'object' ? cfg.mini_cards : {},
    };
}
function exposedFields(fields=currentConfig?.fields || []){
    return (fields || []).filter(f => f.enabled !== false && f.hidden !== true);
}
...
// —— ComfyUI 后端地址管理 ——
let comfyInstances = [];
let comfyInstanceStatusMap = {};
let comfyInstancesSource = 'COMFYUI_INSTANCES';
let comfyPlatformOrigin = '';
const comfyEnvHintEl = document.getElementById('comfyEnvHint');

function renderComfyEnvironmentHint(){
    if(!comfyEnvHintEl) return;
    const activeOrigin = comfyPlatformOrigin || window.location.origin || '';
    const joinedInstances = comfyInstances.length ? comfyInstances.join(', ') : '未配置';
    const source = comfyInstancesSource || 'COMFYUI_INSTANCES';
    const envTag = /(^|\/\/)(192\.168\.1\.60|localhost|127\.0\.0\.1)(:3000)(\/|$)/.test(activeOrigin) ? '（新平台）' : '';
    comfyEnvHintEl.textContent = `当前后台：${activeOrigin || '未知'}${envTag} · 来源：${source} · 实例：${joinedInstances}`;
}

function normalizeComfyBaseUrl(addr){
    const value = String(addr || '').trim().replace(/\/+$/g, '');
    if(!value) return '';
    return /^https?:\/\//i.test(value) ? value : `http://${value}`;
}
async function loadComfyStatusMap(){
    const res = await fetch('/api/comfyui/status');
    if(!res.ok) throw new Error((await res.json()).detail || '状态检测失败');
    const data = await res.json();
    return Object.fromEntries((data.instances || []).map(item => [item.address, item]));
}
async function refreshComfyInstanceStatus(){
    const cleaned = comfyInstances.map(s => String(s || '').trim()).filter(Boolean);
    if(!cleaned.length){
        comfyInstanceStatusMap = {};
        const summary = document.getElementById('comfyInstancesSummary');
        if(summary) summary.textContent = '实例状态：未配置';
        renderComfyEnvironmentHint();
        renderComfyInstances();
        return;
    }
    let entries = [];
    try {
        const statusMap = await loadComfyStatusMap();
        entries = cleaned.map(addr => [addr, statusMap[addr] || { ok:false, reason:'未保存或未检测', base_url:normalizeComfyBaseUrl(addr) }]);
    } catch(err){
        entries = cleaned.map(addr => [addr, { ok:false, reason:err?.message || '状态检测失败', base_url:normalizeComfyBaseUrl(addr) }]);
    }
    comfyInstanceStatusMap = Object.fromEntries(entries);
    const okCount = entries.filter(([, state]) => state.ok).length;
    const failCount = entries.length - okCount;
    const summary = document.getElementById('comfyInstancesSummary');
    if(summary){
        summary.textContent = failCount
            ? `实例状态：${okCount}/${entries.length} 可用，${failCount} 个不可用（请检查 Base URL、端口或后端网络）`
            : `实例状态：${okCount}/${entries.length} 可用`;
    }
    renderComfyEnvironmentHint();
    renderComfyInstances();
}
async function loadComfyInstances(){
    try {
        ...
        comfyInstancesSource = String(data.source || 'COMFYUI_INSTANCES');
        comfyPlatformOrigin = String(data.platform_origin || '').trim();
        renderComfyEnvironmentHint();
        await refreshComfyInstanceStatus();
...
function renderComfyInstances(){
    ...
    el.innerHTML = comfyInstances.map((addr, i) => {
        const state = comfyInstanceStatusMap[addr];
        const statusClass = state ? (state.ok ? 'is-ok' : 'is-fail') : '';
        const statusText = state ? (state.ok ? '在线' : '失败') : '未检测';
        return `
        <div style="display:flex;align-items:center;gap:6px;padding:4px;border:1px solid var(--line);border-radius:9px;background:var(--soft)">
            ...
            <span class="comfy-instance-status ${statusClass}">${statusText}</span>
            <button class="opt-del" type="button" onclick="removeComfyInstance(${i})" title="删除"><i data-lucide="x" class="w-3 h-3"></i></button>
        </div>
        <div style="margin:-3px 4px 2px 28px;font-size:10px;line-height:1.45;color:var(--faint)">Base URL: ${escapeHtml(normalizeComfyBaseUrl(addr) || '-')} · ${escapeHtml(comfyInstanceStatusMap[addr]?.reason || '未检测')}</div>
    `;
    }).join('');
    refreshIcons();
}
...
function renderResourceRootState(payload){
    const detection = payload?.detection || null;
    resourceRootState = detection;
    if(resourceRootInput){
        resourceRootInput.value = payload?.resource_root || '';
    }
    if(!resourceRootSummaryEl || !resourceRootDirsEl) return;
    if(!detection || !payload?.configured){
        resourceRootSummaryEl.textContent = '资源盘状态：未配置（请先填写 60 盘根目录）';
        resourceRootDirsEl.innerHTML = '<div class="import-result-empty">待检测目录结构</div>';
        return;
    }
    const disk = detection.disk || {};
    const summary = detection.summary || {};
    const base = `资源盘状态：${detection.message || '未知'} · 读 ${detection.readable ? '是' : '否'} / 写 ${detection.writable ? '是' : '否'} · 剩余 ${formatBytesGiB(disk.free_bytes)} / 总计 ${formatBytesGiB(disk.total_bytes)}`;
    const suffix = summary.subdir_total
        ? ` · 子目录缺失 ${Number(summary.subdir_missing || 0)}`
        : '';
    resourceRootSummaryEl.textContent = base + suffix;
    const dirs = Array.isArray(detection.suggested_subdirs) ? detection.suggested_subdirs : [];
    if(!dirs.length){
        resourceRootDirsEl.innerHTML = '<div class="import-result-empty">未返回目录检测结果</div>';
        return;
    }
    resourceRootDirsEl.innerHTML = `
        <div class="resource-dir-list">
            ${dirs.map(item => `
                <div class="resource-dir-item">
                    <div class="resource-dir-path">${escapeHtml(item.relative_path || '-')}</div>
                    <span class="resource-dir-state ${resourceDirStatusClass(item.status)}">${resourceDirStatusLabel(item.status)}${item.created ? '（已创建）' : ''}</span>
                </div>
            `).join('')}
        </div>
    `;
...
async function loadResourceRoot(){
    try {
        const res = await fetch('/api/resource-root');
        const data = await res.json();
        if(!res.ok) throw new Error(data.detail || '读取资源根目录失败');
        renderResourceRootState(data);
    } catch(e){
        if(resourceRootSummaryEl) resourceRootSummaryEl.textContent = `资源盘状态：${e.message || '检测失败'}`;
    }
}

async function saveResourceRoot(){
    const value = String(resourceRootInput?.value || '').trim();
    setStatus('保存 60 盘资源根目录...');
    try {
        const res = await fetch('/api/resource-root', {
            method:'PUT',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({ resource_root:value, create_missing:false }),
        });
        const data = await res.json();
...
async function detectResourceRoot(createMissing=false){
    setStatus(createMissing ? '检测并创建目录中...' : '检测 60 盘资源中心...');
    try {
        const res = await fetch('/api/resource-root/detect', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
        ...
        renderResourceRootState(data);
        setStatus(createMissing ? '目录检测并创建完成' : '目录检测完成');
    } catch(e){
    ...
    listEl.innerHTML = workflows.map(w => `
        <button class="workflow-card ${w.name===selectedName?'active':''}" type="button" onclick="selectWorkflow('${escapeHtml(w.name)}')">
            ...
            <span class="min-w-0" style="flex:1">
                ...
                <div class="workflow-meta">${escapeHtml(w.category ? `${w.category} · ` : '')}${tf('comfy.fieldCount', {count:w.field_count})}${w.total_field_count != null ? ` / ${w.total_field_count}` : ''}</div>
            </span>
            ${w.enabled === false ? `<span class="builtin-badge">停用</span>` : ''}
...
async function selectWorkflow(name){
    ...
    try {
        ...
        const data = await res.json();
        if(!res.ok) throw new Error(data.detail || tr('comfy.openFailed'));
        currentWorkflow = data.workflow;
        currentConfig = normalizeConfig(data.config, name.replace('.json',''));
...
function fieldFor(node, input){
    return currentConfig.fields.find(f => f.node === node && f.input === input && f.enabled !== false && f.hidden !== true);
}
function storedFieldFor(node, input){
...
function toggleField(node, input){
    ...
    if(existing){
        ...
        const stored = storedFieldFor(node, input);
        if(stored){
            stored.enabled = true;
            stored.hidden = false;
            if(stored.default !== undefined && stored.default !== null) previewValues[stored.id] = stored.default;
            renderEditor();
            renderPreview();
...
function updateField(fieldId, key, value){
    ...
    if(key === 'enabled' || key === 'hidden' || key === 'required'){
        renderEditor();
        renderPreview();
...
function updateWorkflowMeta(key, value){
    if(!currentConfig) return;
    currentConfig[key] = value;
    const item = workflows.find(w => w.name === selectedName);
    if(item && ['enabled','category','description','thumbnail'].includes(key)) item[key] = value;
    renderList();
}
...
function renderEditor(){
    ...
    const enabledInput = document.getElementById('workflowEnabledInput');
    const categoryInput = document.getElementById('workflowCategoryInput');
    const thumbnailInput = document.getElementById('workflowThumbnailInput');
    const descriptionInput = document.getElementById('workflowDescriptionInput');
    if(enabledInput) enabledInput.checked = currentConfig.enabled !== false;
    if(categoryInput) categoryInput.value = currentConfig.category || '';
    if(thumbnailInput) thumbnailInput.value = currentConfig.thumbnail || '';
    if(descriptionInput) descriptionInput.value = currentConfig.description || '';
    subEl.textContent = tf('comfy.nodeStats', {nodes:Object.keys(currentWorkflow).length, fields:exposedFields().length}) + ` · 候选 ${currentConfig.fields.length}` + (isBuiltin ? ` · ${tr('comfy.builtin')}` : '');
...
function renderGraph(){
    ...
    const nodesHtml = Object.entries(currentWorkflow).map(([id, node]) => {
        ...
        const exposedCount = exposedFields().filter(f => f.node === id).length;
...
function renderExtras(f){
    const flags = `<div class="field-flags-row">
        <label class="field-flag" onclick="event.stopPropagation()"><input type="checkbox" ${f.required === true ? 'checked' : ''} onchange="updateField('${f.id}','required',this.checked)">必填</label>
        <label class="field-flag" onclick="event.stopPropagation()"><input type="checkbox" ${f.hidden === true ? 'checked' : ''} onchange="updateField('${f.id}','hidden',this.checked)">后台隐藏</label>
    </div>`;
    if(f.type === 'slider' || f.type === 'number'){
        const randomToggle = f.type === 'number'
            ...
            : '';
        return `${flags}<div class="extras-row">
    ...
    if(f.type === 'dropdown'){
        ...
        return `${flags}<div class="extras-row" style="flex-direction:column;align-items:stretch;gap:6px">
...
function renderMiniCanvasPreview(target = previewCard, large = false){
    if(!target) return;
    const visibleFields = exposedFields();
    const promptFields = visibleFields.filter(f => fieldKind(f) === 'prompt');
    const imageFields = visibleFields.filter(f => fieldKind(f) === 'image');
    const videoFields = visibleFields.filter(f => fieldKind(f) === 'video');
    const audioFields = visibleFields.filter(f => fieldKind(f) === 'audio');
    const settingFields = visibleFields.filter(f => fieldKind(f) === 'setting');
...
function fieldsFromMiniCanvas(){
    ...
    mediaKinds.forEach(kind => {
        const mediaFields = visibleFields.filter(f => fieldKind(f) === kind);
...
function renderInstallActionRows(actions, type, customNodes){
    const items = (actions || []).filter(item => item.type === type);
    if(!items.length) return '<div class="install-empty">暂无</div>';
    return items.map(action => {
        const id = escapeAttr(action.id || '');
        const status = escapeHtml(installStatusLabel(action.status));
        const title = escapeHtml(action.title || action.value || action.class_type || '-');
        const note = escapeHtml(action.note || '');
        const target = escapeHtml(action.target_relative_path || action.target_path || '');
        if(type === 'model_download'){
            const canInput = !['blocked','skipped','done'].includes(action.status);
            return `
                <div class="install-action-block">
                    <div class="install-action-row">
                        <div class="install-action-main">
                            <div class="install-action-title">${title}</div>
                            <div class="install-action-meta">${escapeHtml(action.category || '-')} ${target ? `· ${target}` : ''}</div>
                            ${note ? `<div class="install-action-note">${note}</div>` : ''}
                        </div>
                        <span class="install-status ${escapeAttr(action.status || '')}">${status}</span>
                        <input id="installInput_${id}" class="small-input install-url-input" type="url" placeholder="https://.../model.safetensors" ${canInput ? '' : 'disabled'}>
                        <button class="install-find-btn" type="button" onclick="findModelDownloadCandidates('${id}')" ${canInput ? '' : 'disabled'} title="查找候选下载链接">
                            <i data-lucide="search" class="w-3.5 h-3.5"></i><span>查找链接</span>
                        </button>
                    </div>
                    <div id="modelCandidates_${id}" class="model-candidates-panel"></div>
                </div>
            `;
        }
        const canRepo = customNodes?.available && !['blocked','skipped','done'].includes(action.status);
        return `
            <div class="install-action-row">
                <div class="install-action-main">
                    <div class="install-action-title">${title}</div>
                    <div class="install-action-meta">${escapeHtml(action.class_type || '')}</div>
                    ${note ? `<div class="install-action-note">${note}</div>` : ''}
                </div>
                <span class="install-status ${escapeAttr(action.status || '')}">${status}</span>
                <input id="installInput_${id}" class="small-input install-url-input" type="url" placeholder="https://github.com/owner/repo" ${canRepo ? '' : 'disabled'}>
            </div>
        `;
    }).join('');
}

function findInstallActionById(actionId){
    const actions = Array.isArray(lastInstallPlan?.actions) ? lastInstallPlan.actions : [];
    return actions.find(item => String(item?.id || '') === String(actionId || '')) || null;
}

function renderModelCandidateItems(actionId, payload){
    const candidates = Array.isArray(payload?.candidates) ? payload.candidates : [];
    const errors = Array.isArray(payload?.errors) ? payload.errors : [];
    if(!candidates.length){
        const errorText = errors.length ? errors.slice(0, 3).join('；') : (payload?.message || '没有找到候选，请手动填写 URL');
        return `<div class="model-candidate-empty">${escapeHtml(errorText)}</div>`;
    }
    return `
        <div class="model-candidate-head">${escapeHtml(payload?.message || '候选下载链接')} · ${escapeHtml(payload?.query || '')}</div>
        ${candidates.map(item => {
            const url = escapeAttr(item.url || '');
            const source = escapeHtml(item.source || '-');
            const title = escapeHtml(item.title || item.filename || item.url || '-');
            const meta = [item.repo, item.filename, item.score ? `score ${item.score}` : '', item.note].filter(Boolean).join(' · ');
            return `
                <div class="model-candidate-row">
                    <div class="model-candidate-main">
                        <div class="model-candidate-title">${title}</div>
                        <div class="model-candidate-meta">${source}${meta ? ` · ${escapeHtml(meta)}` : ''}</div>
                        <div class="model-candidate-url">${escapeHtml(item.url || '')}</div>
                    </div>
                    <button class="install-mini-btn" type="button" data-url="${url}" onclick="useModelCandidateUrl('${escapeAttr(actionId)}', this.dataset.url)">使用</button>
                    <button class="install-mini-btn primary" type="button" data-url="${url}" onclick="downloadModelCandidateUrl('${escapeAttr(actionId)}', this.dataset.url)">下载</button>
                </div>
            `;
        }).join('')}
        ${errors.length ? `<div class="model-candidate-errors">${escapeHtml(errors.slice(0, 3).join('；'))}</div>` : ''}
    `;
}

async function findModelDownloadCandidates(actionId){
    const action = findInstallActionById(actionId);
    const panel = document.getElementById(`modelCandidates_${actionId}`);
    if(!action || !panel) return;
    panel.innerHTML = '<div class="model-candidate-empty">正在查找候选链接...</div>';
    setStatus('查找模型候选链接...');
    try {
        const res = await fetch('/api/workflow-install/model-candidates', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({ action }),
        });
        const data = await res.json();
        if(!res.ok) throw new Error(data.detail || '查找候选失败');
        panel.innerHTML = renderModelCandidateItems(actionId, data);
        setStatus(data.candidates?.length ? '已找到候选链接' : '未找到候选链接');
        refreshIcons();
    } catch(e){
        panel.innerHTML = `<div class="model-candidate-empty">查找失败：${escapeHtml(e.message || '未知错误')}</div>`;
        setStatus('查找候选失败');
    }
}

function useModelCandidateUrl(actionId, url){
    const input = document.getElementById(`installInput_${actionId}`);
    if(input && url){
        input.value = url;
        setStatus('已填入候选下载链接');
    }
...
async function downloadModelCandidateUrl(actionId, url){
    const action = findInstallActionById(actionId);
    if(!action || !url) return;
    useModelCandidateUrl(actionId, url);
    clearTimeout(workflowInstallPollTimer);
    setStatus('创建模型下载任务...');
    try {
        const res = await fetch('/api/workflow-install/tasks', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({ actions:[{...action, source_url:url, executable:true, status:'ready'}] }),
        });
        const data = await res.json();
        if(!res.ok) throw new Error(data.detail || '创建模型下载任务失败');
        renderWorkflowInstallTask(data);
        setStatus('模型下载任务已创建');
        pollWorkflowInstallTask(data.task_id);
    } catch(e){
        alert(e.message || '创建模型下载任务失败');
        setStatus('模型下载任务创建失败');
    }
}

function renderInstallPlan(plan){
    const actions = Array.isArray(plan?.actions) ? plan.actions : [];
    if(!actions.length) return '<div class="install-empty">当前没有可安装项</div>';
    const customNodes = plan?.custom_nodes || {};
    const customNodesLine = customNodes.configured
        ? `${escapeHtml(customNodes.path || '-')} · ${escapeHtml(customNodes.message || '')}`
        : '未配置 AITOOL_COMFYUI_CUSTOM_NODES_DIR / COMFYUI_CUSTOM_NODES_DIR';
    return `
        <div class="install-plan-box">
            <div class="install-plan-head">
                <div class="install-plan-title">安装计划</div>
                <div class="install-plan-actions">
                    <button class="upload-btn install-run-btn install-auto-btn" type="button" onclick="runAutoModelDownloads()">
                        <i data-lucide="download" class="w-3.5 h-3.5"></i><span>一键下载高置信模型</span>
                    </button>
                    <button class="upload-btn install-run-btn" type="button" onclick="runWorkflowInstallPlan()">
                        <i data-lucide="play" class="w-3.5 h-3.5"></i><span>执行可用安装</span>
                    </button>
                </div>
            </div>
            <div class="install-section-title">模型下载</div>
            <div class="install-action-list">${renderInstallActionRows(actions, 'model_download', customNodes)}</div>
            <div class="install-section-title">自定义节点</div>
            <div class="install-custom-note">${customNodesLine}</div>
            <div class="install-action-list">${renderInstallActionRows(actions, 'custom_node_install', customNodes)}</div>
            <div id="workflowInstallTaskPanel" class="install-task-panel"></div>
        </div>
    `;
}

function collectWorkflowInstallActions(){
    const plan = lastInstallPlan || {};
    const customNodes = plan.custom_nodes || {};
    const actions = Array.isArray(plan.actions) ? plan.actions : [];
    const selected = [];
    actions.forEach(action => {
        if(!action || ['blocked','skipped','done'].includes(action.status)) return;
        const input = document.getElementById(`installInput_${action.id || ''}`);
        const value = String(input?.value || '').trim();
        if(!value) return;
        if(action.type === 'model_download'){
            selected.push({...action, source_url:value, executable:true, status:'ready'});
        } else if(action.type === 'custom_node_install' && customNodes.available){
            selected.push({...action, repo_url:value, executable:true, status:'ready'});
        }
    });
    return selected;
}

function renderAutoModelDownloadSummary(summary){
    if(!summary) return '';
    const selected = Array.isArray(summary.selected) ? summary.selected : [];
    const manual = Array.isArray(summary.manual_required) ? summary.manual_required : [];
    const skipped = Array.isArray(summary.skipped) ? summary.skipped : [];
    const errors = Array.isArray(summary.errors) ? summary.errors : [];
    const countLine = `已选择 ${selected.length} 个高置信同名模型 · ${manual.length} 个需人工确认 · ${skipped.length} 个跳过`;
    const selectedHtml = selected.length ? `
        <div class="install-auto-list">
            ${selected.slice(0, 5).map(item => `
                <div class="install-auto-item ok">
                    <span>${escapeHtml(item.target_filename || item.title || '-')}</span>
                    <small>${escapeHtml(item.source || '-')} · score ${escapeHtml(item.score || 0)}</small>
                </div>
            `).join('')}
            ${selected.length > 5 ? `<div class="install-auto-more">另有 ${selected.length - 5} 个已选择</div>` : ''}
        </div>
    ` : '';
    const manualHtml = manual.length ? `
        <div class="install-auto-list">
            ${manual.slice(0, 6).map(item => `
                <div class="install-auto-item manual">
                    <span>${escapeHtml(item.target_filename || item.title || '-')}</span>
                    <small>${escapeHtml(item.reason || '需要人工填写 URL')}</small>
                </div>
            `).join('')}
            ${manual.length > 6 ? `<div class="install-auto-more">另有 ${manual.length - 6} 个需要人工确认</div>` : ''}
        </div>
    ` : '';
    return `
        <div class="install-auto-summary">
            <div class="install-auto-head">${escapeHtml(summary.message || countLine)}</div>
            <div class="install-auto-counts">${escapeHtml(countLine)}</div>
            ${selectedHtml}
            ${manualHtml}
            ${errors.length ? `<div class="install-auto-errors">${escapeHtml(errors.slice(0, 3).join('；'))}</div>` : ''}
        </div>
    `;
}

function workflowTaskProgressItems(task){
    const actions = Array.isArray(task?.actions) ? task.actions : [];
    const progressMap = task?.progress || {};
    return actions
        .filter(action => action?.type === 'model_download')
        .map(action => {
            const progress = progressMap[action.id] || action.progress || null;
            return { action, progress };
        })
        .filter(item => item.progress || ['running','done','failed','skipped'].includes(item.action?.status));
}

function renderWorkflowInstallProgress(task){
    const items = workflowTaskProgressItems(task);
    if(!items.length) return '';
    return `
        <div class="install-progress-list">
            ${items.map(({action, progress}) => {
                const p = progress || {};
                const rawPercent = p.percent == null ? null : Number(p.percent);
                const hasPercent = Number.isFinite(rawPercent);
                const percent = hasPercent ? Math.max(0, Math.min(100, rawPercent)) : null;
                const downloaded = formatDownloadBytes(p.downloaded_bytes);
                const total = Number(p.total_bytes || 0) > 0 ? formatDownloadBytes(p.total_bytes) : '';
                const sizeText = total ? `${downloaded} / ${total}` : `${downloaded} 已下载`;
                const speed = formatDownloadSpeed(p.speed_bytes_per_sec);
                const phase = p.phase || p.status || action.status || '';
                const target = p.target_relative_path || action.target_relative_path || action.title || action.value || '';
                const name = fileBaseName(target) || action.title || '-';
                const note = p.error || action.note || '';
                return `
                    <div class="install-progress-row ${escapeAttr(phase)}">
                        <div class="install-progress-top">
                            <div class="install-progress-name">${escapeHtml(name)}</div>
                            <div class="install-progress-percent">${hasPercent ? `${percent.toFixed(percent >= 10 || percent === 0 || percent === 100 ? 0 : 1)}%` : escapeHtml(installStatusLabel(phase) || '下载中')}</div>
                        </div>
                        <div class="install-progress-bar"><span style="width:${hasPercent ? percent : 0}%"></span></div>
                        <div class="install-progress-meta">
                            <span>${escapeHtml(sizeText)}</span>
                            ${speed ? `<span>${escapeHtml(speed)}</span>` : ''}
                            <span>${escapeHtml(installStatusLabel(phase) || phase)}</span>
                        </div>
                        ${note && ['failed','done','skipped'].includes(String(phase)) ? `<div class="install-progress-note">${escapeHtml(note)}</div>` : ''}
                    </div>
                `;
            }).join('')}
        </div>
...
function renderWorkflowInstallTask(task){
    const panel = document.getElementById('workflowInstallTaskPanel');
    if(!panel) return;
    const autoSummary = task?.auto_model_downloads || workflowAutoDownloadSummary;
    if(task?.auto_model_downloads) workflowAutoDownloadSummary = task.auto_model_downloads;
    const logs = Array.isArray(task?.logs) ? task.logs : [];
    const results = Array.isArray(task?.results) ? task.results : [];
    panel.innerHTML = `
        ${renderAutoModelDownloadSummary(autoSummary)}
        <div class="install-task-status">任务状态：${escapeHtml(installStatusLabel(task?.status) || task?.status || '-')}</div>
        ${renderWorkflowInstallProgress(task)}
        ${results.length ? `<div class="install-task-results">${results.map(item => `
            <div>${escapeHtml(item.title || item.value || item.class_type || item.id || '-')}：${escapeHtml(installStatusLabel(item.status))}${item.note ? ` · ${escapeHtml(item.note)}` : ''}</div>
        `).join('')}</div>` : ''}
        <pre class="install-task-log">${logs.map(item => `[${item.time || ''}] ${item.message || ''}`).map(escapeHtml).join('\n') || '等待日志...'}</pre>
    `;
    refreshIcons();
}

async function pollWorkflowInstallTask(taskId){
    clearTimeout(workflowInstallPollTimer);
    try {
        const res = await fetch(`/api/workflow-install/tasks/${encodeURIComponent(taskId)}`);
        const data = await res.json();
        if(!res.ok) throw new Error(data.detail || '查询安装任务失败');
        renderWorkflowInstallTask(data);
        if(!['done','failed'].includes(data.status)){
            workflowInstallPollTimer = setTimeout(() => pollWorkflowInstallTask(taskId), 1500);
        } else {
            setStatus(data.status === 'done' ? '安装任务完成' : '安装任务失败');
        }
    } catch(e){
        const panel = document.getElementById('workflowInstallTaskPanel');
        if(panel) panel.innerHTML = `<div class="install-task-status">查询失败：${escapeHtml(e.message || '未知错误')}</div>`;
    }
}

async function runAutoModelDownloads(){
    const actions = Array.isArray(lastInstallPlan?.actions) ? lastInstallPlan.actions : [];
    const modelActions = actions.filter(action => action?.type === 'model_download' && !['blocked','skipped','done'].includes(action.status));
    if(!modelActions.length){
        alert('当前没有可一键查找下载的缺失模型');
        return;
    }
    clearTimeout(workflowInstallPollTimer);
    workflowAutoDownloadSummary = null;
    const panel = document.getElementById('workflowInstallTaskPanel');
    if(panel){
        panel.innerHTML = `
            <div class="install-auto-summary">
                <div class="install-auto-head">正在查找高置信同名模型...</div>
                <div class="install-auto-counts">会自动跳过弱匹配、非同名和无模型后缀的条目</div>
            </div>
        `;
    }
    setStatus('正在查找高置信模型候选...');
    try {
        const res = await fetch('/api/workflow-install/auto-model-downloads', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({ actions }),
        });
        const data = await res.json();
        if(!res.ok) throw new Error(data.detail || '一键下载模型失败');
        workflowAutoDownloadSummary = data;
        if(data.task){
            renderWorkflowInstallTask(data.task);
            setStatus(data.selected?.length ? `已选择 ${data.selected.length} 个模型，开始下载` : '没有可自动下载的高置信模型');
            pollWorkflowInstallTask(data.task_id);
        } else {
            if(panel) panel.innerHTML = renderAutoModelDownloadSummary(data);
            setStatus('没有可自动下载的高置信模型，请人工确认 URL');
        }
    } catch(e){
        if(panel) panel.innerHTML = `<div class="install-task-status">一键下载失败：${escapeHtml(e.message || '未知错误')}</div>`;
        alert(e.message || '一键下载模型失败');
        setStatus('一键下载模型失败');
    }
}

async function runWorkflowInstallPlan(){
    const actions = collectWorkflowInstallActions();
    if(!actions.length){
        alert('请先为缺失模型填写下载 URL，或为自定义节点填写 GitHub 仓库 URL');
        return;
    }
    clearTimeout(workflowInstallPollTimer);
    workflowAutoDownloadSummary = null;
    ...
    try {
        ...
        const data = await res.json();
        if(!res.ok) throw new Error(data.detail || '创建安装任务失败');
        renderWorkflowInstallTask(data);
        setStatus('安装任务已创建');
        pollWorkflowInstallTask(data.task_id);
    } catch(e){
        alert(e.message || '创建安装任务失败');
        setStatus('安装任务创建失败');
    }
}

function renderImportWizardResult(payload, level='ok'){
    if(!importWizardResultEl) return;
    clearTimeout(workflowInstallPollTimer);
    workflowAutoDownloadSummary = null;
    lastInstallPlan = payload?.install_plan || null;
    const statusText = level === 'fail' ? '失败' : level === 'warn' ? '需人工处理' : '完成';
    const workflow = payload?.workflow || {};
    const compatibility = Array.isArray(payload?.compatibility) ? payload.compatibility : [];
    const modelDeps = Array.isArray(payload?.model_dependencies) ? payload.model_dependencies : [];
    const planItems = Array.isArray(payload?.plan_items) ? payload.plan_items : [];
    const resourceRoot = payload?.resource_root || {};
    const compRows = compatibility.length
        ? compatibility.map(item => `
            <tr>
                <td>${escapeHtml(item.address || '-')}</td>
                <td>${item.compatible ? '兼容' : '不兼容'}</td>
                <td>${escapeHtml((item.missing_nodes || []).slice(0, 6).join(', ') || '-')}</td>
            </tr>
        `).join('')
        : '<tr><td colspan="3">暂无实例数据</td></tr>';
    const modelRows = modelDeps.length
        ? modelDeps.map(item => `
            <tr>
                <td>${escapeHtml(item.input_key || '-')}</td>
                <td>${escapeHtml(item.value || '-')}</td>
                <td>${escapeHtml(item.category || '-')}</td>
                <td><span class="model-status-pill ${modelStatusClass(item.status)}">${modelStatusLabel(item.status)}</span></td>
                <td>${escapeHtml(item.path || item.relative_path || '-')}</td>
            </tr>
        `).join('')
        : '<tr><td colspan="5">未识别到常见模型字段</td></tr>';
    const resourceLine = resourceRoot?.configured
        ? `资源盘：${escapeHtml(resourceRoot.resource_root || '-')} · ${escapeHtml(resourceRoot.message || '已配置')}`
        : '资源盘：未配置（模型依赖保持待确认）';
    importWizardResultEl.innerHTML = `
        <div class="import-result-box">
            <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap">
                <div class="import-result-title">${escapeHtml(payload?.message || '导入分析结果')}</div>
                <span class="import-result-status ${level}">${statusText}</span>
            </div>
            <div>工作流：${escapeHtml(workflow.title || workflow.name || '-')} · 节点 ${Number(workflow.node_count || 0)} · class_type ${Number(workflow.required_class_count || 0)}</div>
            <div>${resourceLine}</div>
            <div style="font-weight:800;color:var(--text)">节点兼容性</div>
            <table class="import-table">
                <thead><tr><th>实例</th><th>状态</th><th>缺失节点</th></tr></thead>
                <tbody>${compRows}</tbody>
            </table>
            <div style="font-weight:800;color:var(--text)">模型依赖清单</div>
            <table class="import-table">
                <thead><tr><th>输入键</th><th>值</th><th>分类</th><th>状态</th><th>命中路径</th></tr></thead>
                <tbody>${modelRows}</tbody>
            </table>
            <div style="font-weight:800;color:var(--text)">安装计划 / 人工事项</div>
            <ul class="import-list">
                ${(planItems.length ? planItems : ['无']).map(item => `<li>${escapeHtml(item)}</li>`).join('')}
            </ul>
            ${renderInstallPlan(lastInstallPlan)}
        </div>
    `;
}

async function onImportJsonFile(event){
    const file = event.target.files?.[0];
    event.target.value = '';
    if(!file) return;
    try {
        const text = await file.text();
        if(importJsonInput) importJsonInput.value = text;
        if(importWorkflowNameInput && !importWorkflowNameInput.value.trim()){
            importWorkflowNameInput.value = file.name.replace(/\.json$/i, '');
        }
        setStatus('已加载 JSON 文件，点击“分析并导入”继续');
    } catch(e){
        alert('读取 JSON 文件失败');
    }
}

async function runWorkflowImportWizard(){
    const runBtn = document.querySelector('.import-submit-btn');
    const runninghubSource = String(importSourceInput?.value || '').trim();
    const jsonText = String(importJsonInput?.value || '').trim();
    const workflowName = String(importWorkflowNameInput?.value || '').trim();
    let body = {
        source_type: '',
        source_value: '',
        workflow_json: {},
    ...
    if(jsonText){
        ...
        catch { alert('JSON 格式无效，请检查后重试'); return; }
        body.source_type = 'workflow_json';
        body.workflow_json = parsed;
        body.source_value = '';
    } else if(runninghubSource){
        body.source_type = 'runninghub_ref';
        body.source_value = runninghubSource;
    } else {
        alert('请粘贴 RunningHub URL/ID，或粘贴/上传 workflow JSON');
        return;
    }
    if(runBtn){
    ...
    setStatus('导入预检中...');
    try {
        const res = await fetch('/api/workflows/import/plan', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify(body),
        });
        const data = await res.json();
        if(!res.ok) throw new Error(data.detail || '导入分析失败');
        const level = data.success === false ? 'warn' : 'ok';
        renderImportWizardResult(data, level);
        if(data.saved_workflow?.name){
            await loadList();
            await selectWorkflow(data.saved_workflow.name);
            setStatus(`导入完成：${data.saved_workflow.name}`);
            new BroadcastChannel('studio-api').postMessage({ type: 'workflows-changed' });
        } else {
            setStatus(data.message || '导入分析完成');
        }
    } catch(e){
        renderImportWizardResult({ message: e.message || '导入分析失败', compatibility: [], model_dependencies: [], plan_items: ['请检查输入后重试'], workflow:{} }, 'fail');
        setStatus('导入分析失败');
    } finally {
        if(runBtn){
