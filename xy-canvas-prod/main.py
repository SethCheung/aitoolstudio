...
def verify_password(password, stored_hash):
    """支持 xy-canvas SHA256 和旧 AIToolStudio 密码哈希。"""
    raw = str(stored_hash or "")
    if raw.startswith("pbkdf2_sha256$"):
        try:
            algo, iterations_text, salt_text, digest_hex = raw.split("$", 3)
            ...
            return False
    if raw.startswith(("$2a$", "$2b$", "$2y$")) and crypt:
        try:
            digest = crypt.crypt(str(password or ""), raw)
            return bool(digest) and hmac.compare_digest(digest, raw)
        except Exception:
            return False
    return hmac.compare_digest(raw, hash_password(password))
...
def public_provider(provider):
    ...
    return {
        ...
        "key_preview": masked_key_preview(key_value),
        "key_env": key_env if key_value else "",
    }
...
PROMPT_BACKEND_MAP: Dict[str, str] = {}
OBJECT_INFO_CACHE: Dict[str, Dict[str, Any]] = {}
OBJECT_INFO_CACHE_TS: Dict[str, float] = {}
OBJECT_INFO_CACHE_TTL = 60
...
def _normalize_comfyui_addr(value: Optional[str]) -> Optional[str]:
    if not value:
        return None
    ...
    addr = parsed.netloc or parsed.path
    return addr.strip().rstrip("/") or None

def resolve_allowed_comfyui_addr(value: Optional[str]) -> Optional[str]:
    addr = _normalize_comfyui_addr(value)
    if not addr:
        return None
    if addr not in COMFYUI_INSTANCES:
        raise HTTPException(status_code=400, detail=f"ComfyUI 地址不在允许列表：{addr}")
    return addr

def requested_comfyui_addr(request: Request) -> Optional[str]:
    return resolve_allowed_comfyui_addr(
        request.headers.get("X-ComfyUI-Server-Url")
        or request.query_params.get("server_url")
        or request.query_params.get("server")
        or request.query_params.get("instance_id")
    )

def extract_prompt_classes(payload: bytes) -> set:
    try:
...
def get_comfyui_object_info_classes(addr: str) -> Optional[set]:
    now = time.time()
    cached = OBJECT_INFO_CACHE.get(addr)
    if cached is not None and now - OBJECT_INFO_CACHE_TS.get(addr, 0) < OBJECT_INFO_CACHE_TTL:
        return set(cached.keys())
    try:
        resp = requests.get(f"http://{addr}/object_info", timeout=5)
        resp.raise_for_status()
        data = resp.json()
        if isinstance(data, dict):
            OBJECT_INFO_CACHE[addr] = data
            OBJECT_INFO_CACHE_TS[addr] = now
            return set(data.keys())
    except Exception as e:
        print(f"[ComfyUI Proxy] object_info check failed for {addr}: {e}")
    return None

def choose_comfyui_backend_for_prompt(payload: bytes, requested: Optional[str] = None) -> str:
    if requested:
        return requested
    classes = extract_prompt_classes(payload)
    if not classes:
        return COMFYUI_ADDRESS
    default_classes = get_comfyui_object_info_classes(COMFYUI_ADDRESS)
    if default_classes is not None and classes.issubset(default_classes):
        return COMFYUI_ADDRESS
    preferred = []
    if "Dapao_LlamaCaption" in classes:
        preferred = [addr for addr in COMFYUI_INSTANCES if addr.startswith("192.168.1.249:")]
    for addr in preferred + [addr for addr in COMFYUI_INSTANCES if addr not in preferred]:
        available = get_comfyui_object_info_classes(addr)
        if available is not None and classes.issubset(available):
            return addr
    return COMFYUI_ADDRESS

def ordered_comfyui_instances(preferred: Optional[str] = None) -> List[str]:
    ordered = []
    if preferred and preferred in COMFYUI_INSTANCES:
...
def workflow_required_images(workflow: Dict[str, Any]) -> List[str]:
    images = []
    ...
    for node in workflow.values():
        ...
        if isinstance(image_name, str) and image_name and image_name not in images:
            images.append(image_name)
    return images

def backend_supports_prompt_payload(addr: str, payload: bytes) -> bool:
    classes = extract_prompt_classes(payload)
    if not classes:
        return True
    available = get_comfyui_object_info_classes(addr)
    return available is not None and classes.issubset(available)

def ensure_images_on_backend(target_backend: str, required_images: List[str]) -> None:
...
def api_headers(json_body=True):
    ...
    headers = build_api_headers(AI_API_KEY, json_body=json_body)
    return headers

def build_api_headers(api_key, json_body=True):
    if not api_key:
...
def api_base_with_v1(base_url):
    base = (base_url or "").strip().rstrip("/")
    if not base:
        raise HTTPException(status_code=400, detail="API 请求地址不能为空")
    return base if base.endswith("/v1") else f"{base}/v1"

def resolve_image_provider_runtime(provider_id, model=""):
    provider = get_api_provider(provider_id or "comfly")
    if provider:
        api_key = os.getenv(provider_key_env(provider["id"]), "") or globals().get(provider_key_env(provider["id"]), "")
        if not api_key:
            raise HTTPException(status_code=400, detail=f"{provider.get('name') or provider['id']} 未配置 API Key")
        default_model = (provider.get("image_models") or [IMAGE_MODEL])[0]
        protocol = (provider.get("protocol") or "openai").strip().lower()
        return {
            "provider": provider,
            "base_url": api_base_with_v1(provider.get("base_url") or AI_BASE_URL),
            "api_key": api_key,
            "protocol": protocol,
            "model": selected_model(model, default_model),
        }
    return {
        "provider": None,
        "base_url": api_base_with_v1(AI_BASE_URL),
        "api_key": AI_API_KEY,
        "protocol": "openai",
        "model": selected_model(model, IMAGE_MODEL),
    }

def saved_modelscope_api_key():
    if MODELSCOPE_API_KEY.strip():
        return MODELSCOPE_API_KEY.strip()
    for provider in load_api_providers():
        base = (provider.get("base_url") or "").lower()
        protocol = (provider.get("protocol") or "").lower()
        if "modelscope" in base or protocol == "apimart":
            api_key = os.getenv(provider_key_env(provider["id"]), "") or globals().get(provider_key_env(provider["id"]), "").strip()
            if api_key:
                return api_key
    return ""
...
def extract_modelscope_image(data):
    images = data.get("output_images") or []
    if not images and isinstance(data.get("data"), dict):
        images = data["data"].get("output_images") or []
    if images:
        return {"type": "url", "value": images[0]}
    return extract_image(data)

async def wait_for_image_task(client, task_id, base_url=None, api_key=None):
    deadline = time.monotonic() + AI_REQUEST_TIMEOUT
    last_payload = {}
    base = base_url or api_base_with_v1(AI_BASE_URL)
    headers = build_api_headers(api_key or AI_API_KEY)
    while time.monotonic() < deadline:
        response = await client.get(f"{base}/images/tasks/{task_id}", headers=headers)
...
async def wait_for_modelscope_task(client, task_id, base_url, api_key):
    deadline = time.monotonic() + AI_REQUEST_TIMEOUT
    last_payload = {}
    ...
    while time.monotonic() < deadline:
        ...
        response.raise_for_status()
        last_payload = response.json()
        status = str(last_payload.get("task_status") or "").upper()
        if status == "SUCCEED":
            return last_payload
        if status in {"FAILED", "FAIL", "ERROR", "CANCELED", "CANCELLED", "TIMEOUT", "REVOKED"}:
            reason = last_payload.get("error_info") or last_payload.get("message") or last_payload.get("detail") or "生图任务失败"
            raise HTTPException(status_code=502, detail=f"ModelScope 生图任务失败：{reason}")
        await asyncio.sleep(IMAGE_POLL_INTERVAL)
    raise HTTPException(status_code=504, detail=f"ModelScope 生图任务超时，task_id={task_id}")
...
async def generate_ai_image(prompt, size, quality, model, reference_images=None, protocol="openai", base_url=None, api_key=None):
    refs = [ref for ref in (reference_images or []) if ref.get("url")]
    api_base = base_url or api_base_with_v1(AI_BASE_URL)
    auth_key = api_key or AI_API_KEY
    ...
    is_modelscope_async = protocol == "apimart"
    
    async with httpx.AsyncClient(timeout=AI_REQUEST_TIMEOUT) as client:
        if is_modelscope_async:
            payload = {
                "model": model,
                "prompt": prompt,
                "size": size,
            ...
            response = await client.post(
                ...
                headers={**build_api_headers(auth_key), "X-ModelScope-Async-Mode": "true"},
                json=payload,
            )
            ...
            response = await client.post(
                ...
                headers=build_api_headers(auth_key),
                json={
                    "model": model,
                    "messages": messages,
                },
            ...
            try:
                ...
                response = await client.post(f"{api_base}/images/edits", headers=build_api_headers(auth_key, json_body=False), data=data, files=files)
            finally:
                for fh in opened:
                    fh.close()
        else:
        ...
        try:
            ...
            if is_modelscope_async:
                ...
                if task_id:
                    task_result = await wait_for_modelscope_task(client, task_id, api_base, auth_key)
                    return extract_modelscope_image(task_result), task_result
                return extract_modelscope_image(raw), raw
            return extract_image(raw), raw
        except HTTPException:
            task_id = extract_task_id(raw)
            if not task_id:
                raise
        task_result = await wait_for_image_task(client, task_id, api_base, auth_key)
...
async def auth_register_disabled():
    """本地部署：关闭公开注册（由 owner 在 P0 修复 2026-06-04 关闭）"""
...
async def update_config(payload: ConfigUpdateRequest):
    ...
    if env_updates:
        for k, v in env_updates.items(): os.environ[k] = v; save_env_file(env_updates)
...
async def online_image(payload: OnlineImageRequest, token: str = ""):
    username = get_username_by_token(token)
    runtime = resolve_image_provider_runtime(payload.provider_id or "comfly", payload.model)
    model = runtime["model"]
    protocol = runtime["protocol"] if runtime["provider"] else payload.protocol
...
async def poll_angle_cloud(req: CloudPollRequest):
    ...
    clean_token = (req.api_key or saved_modelscope_api_key()).strip()
    if not clean_token:
        raise HTTPException(status_code=400, detail="未提供 ModelScope API Key，请在 API 设置保存魔搭 Key。")
...
async def ms_generate(req: MsGenerateRequest, token: str = ""):
    username = get_username_by_token(token)
    base_url = 'https://api-inference.modelscope.cn/'
    clean_token = saved_modelscope_api_key()
    if not clean_token:
        raise HTTPException(status_code=400, detail="未提供 ModelScope API Key，请在 API 设置保存魔搭 Key。")
...
def generate(req: GenerateRequest, token: str = ""):
    ...
    try:
        ...
        ensure_images_on_backend(target_backend, required_images)
        ...
        for image_name in workflow_required_images(workflow):
            if image_name not in required_images:
                required_images.append(image_name)
        ...
        if not backend_supports_prompt_payload(target_backend, data):
            next_backend = choose_comfyui_backend_for_prompt(data)
            if next_backend and next_backend != target_backend:
                with LOAD_LOCK:
                    if BACKEND_LOCAL_LOAD.get(target_backend, 0) > 0:
                        BACKEND_LOCAL_LOAD[target_backend] -= 1
                    BACKEND_LOCAL_LOAD[next_backend] = BACKEND_LOCAL_LOAD.get(next_backend, 0) + 1
                target_backend = next_backend
        ensure_images_on_backend(target_backend, required_images)

        try:
            post_req = urllib.request.Request(f"http://{target_backend}/prompt", data=data, headers={"Content-Type": "application/json"})
...
async def proxy_comfyui_system_stats(request: Request):
    """代理检查 ComfyUI 服务器状态"""
    target = requested_comfyui_addr(request) or COMFYUI_ADDRESS
    if not target:
...
async def proxy_comfyui_object_info(request: Request):
    """代理 ComfyUI object_info，供独立应用页探测节点能力。"""
    target = requested_comfyui_addr(request) or COMFYUI_ADDRESS
    if not target:
        raise HTTPException(status_code=503, detail="未配置 ComfyUI 地址")
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            resp = await client.get(f"http://{target}/object_info")
        return Response(
...
async def proxy_comfyui_prompt(request: Request):
    """代理提交原生 ComfyUI prompt，兼容旧的 standalone app 页面。"""
    try:
        payload = await request.body()
        target = choose_comfyui_backend_for_prompt(payload, requested_comfyui_addr(request))
        if not target:
            raise HTTPException(status_code=503, detail="未配置 ComfyUI 地址")
        headers = {"Content-Type": request.headers.get("content-type", "application/json")}
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.post(f"http://{target}/prompt", content=payload, headers=headers)
        try:
            result = resp.json()
            prompt_id = result.get("prompt_id")
            if prompt_id:
                PROMPT_BACKEND_MAP[prompt_id] = target
        except Exception:
            pass
        return Response(
            content=resp.content,
        ...
        raise HTTPException(status_code=502, detail=f"ComfyUI 提交失败: {str(e)}")

@app.get("/api/comfyui/history/{prompt_id}")
async def proxy_comfyui_history(prompt_id: str, request: Request):
    """代理读取 ComfyUI history。"""
    preferred = PROMPT_BACKEND_MAP.get(prompt_id) or requested_comfyui_addr(request)
    targets = ordered_comfyui_instances(preferred)
    if not targets:
        raise HTTPException(status_code=503, detail="未配置 ComfyUI 地址")
    last_error = None
    for target in targets:
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                resp = await client.get(f"http://{target}/history/{urllib.parse.quote(prompt_id)}")
            if resp.status_code == 200:
                try:
                    data = resp.json()
                    if prompt_id in data or preferred == target:
                        return Response(
                            content=resp.content,
                            status_code=resp.status_code,
                            media_type=resp.headers.get("content-type", "application/json"),
            ...
            continue
    if last_error:
        raise HTTPException(status_code=502, detail=f"ComfyUI 历史读取失败: {str(last_error)}")
    return Response(content=json.dumps({}), media_type="application/json")

@app.get("/api/comfyui/view")
async def proxy_comfyui_view(filename: str, type: str = "output", subfolder: str = ""):
    """代理查看 ComfyUI 图片输出，避免局域网浏览器直连 127.0.0.1。"""
    ...
    for addr in COMFYUI_INSTANCES:
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                resp = await client.get(f"http://{addr}/view", params=params)
            if resp.status_code == 200:
                return Response(
                    content=resp.content,
                    status_code=resp.status_code,
...
async def proxy_comfyui_upload_image(
    file: Optional[UploadFile] = File(None),
    image: Optional[UploadFile] = File(None),
):
    """代理上传图片到 ComfyUI（兼容 file 和 ComfyUI 原生 image 字段）"""
    upload = file or image
    if upload is None:
        raise HTTPException(status_code=422, detail="缺少上传文件字段 file 或 image")
    content = await upload.read()
    result = None
    for addr in COMFYUI_INSTANCES:
        try:
            files_data = {'image': (upload.filename or f"upload_{int(time.time())}.png", content, upload.content_type or "image/png")}
...
async def proxy_comfyui_ws(websocket: WebSocket, client_id: str = None, server_url: str = None, server: str = None):
    """代理 ComfyUI WebSocket，让前端通过后端获取实时进度"""
    await websocket.accept()
    try:
        requested_target = resolve_allowed_comfyui_addr(server_url or server)
    except HTTPException as e:
        await websocket.send_json({"type": "error", "message": e.detail})
        await websocket.close()
        return
    targets = ordered_comfyui_instances(requested_target) if requested_target else COMFYUI_INSTANCES.copy()
    if not targets:
    ...
    try:
        for target in targets:
            try:
                comfy_sockets.append(await ws_client.connect(f"ws://{target}/ws?clientId={cid}"))
            except Exception as e:
                print(f"[ComfyUI WS Proxy] {target} 连接失败: {e}")
        if not comfy_sockets:
            await websocket.send_json({"type": "error", "message": "所有 ComfyUI WebSocket 均连接失败"})
            await websocket.close()
            return

        send_lock = asyncio.Lock()

        async def forward_to_frontend(comfy_ws):
            try:
        ...
        tasks = [asyncio.create_task(forward_to_frontend(sock)) for sock in comfy_sockets]
        tasks.append(asyncio.create_task(forward_to_comfy()))
...
# --- Canvas Video Task API 注册 (xy-canvas P0 修复,2026-06-04) ---
try:
    import canvas_video
    _cv_ctx = {
        "app": app,
        ...
        "WORKFLOW_DIR": WORKFLOW_DIR if "WORKFLOW_DIR" in dir() else os.path.join(os.path.dirname(__file__), "workflows"),
        "OUTPUT_DIR": OUTPUT_DIR if "OUTPUT_DIR" in dir() else os.path.join(os.path.dirname(__file__), "output"),
        "CLIENT_ID": CLIENT_ID if "CLIENT_ID" in dir() else "xy-canvas-client",
        "COMFYUI_INSTANCES": COMFYUI_INSTANCES,
        "BACKEND_LOCAL_LOAD": BACKEND_LOCAL_LOAD,
        "LOAD_LOCK": LOAD_LOCK,
        "get_best_backend": get_best_backend,
        "ensure_images_on_backend": ensure_images_on_backend,
        "get_comfy_history": get_comfy_history,
        "download_image": download_image,
        "build_output_url": build_output_url,
        "get_canvas_output_dir": get_canvas_output_dir,
        "get_user_output_dir": get_user_output_dir,
        "get_username_by_token": get_username_by_token,
        "load_canvas": load_canvas,
        "workflow_required_images": workflow_required_images,
        "time": time,
        "json": json,
        "asyncio": asyncio,
        "random": random,
    ...
    canvas_video.register_canvas_video_routes(app, _cv_ctx)
    print("[CanvasVideo] Routes registered: /api/canvas-video-tasks")
except Exception as _cv_err:
    print(f"[CanvasVideo] Register failed: {_cv_err}")
    import traceback
    traceback.print_exc()
