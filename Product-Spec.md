...
| **生成数量** | 图片生成支持 `1x / 2x / 4x` 数量切换，前端将数量传给后端 `n` 参数 |
...
| **生成数量选择** | 用户输入单条提示词 → 选择 `1x / 2x / 4x` → 点击生成 → 一次得到对应数量的图片 |
...
| **MiniMax image-01 生图** | 用户在生成框选择 image 分类和 MiniMax 图像模型 → 输入提示词 → 调用 MiniMax image-01 API → 得到另一种风格的生图（作为 ComfyUI 的备选） |
